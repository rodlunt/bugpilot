...
1. **`POST /feedback` requires the `context` object; the worker returns a clean 400 if it is missing (guard added in PR #33, 11-Jul-2026, verified live 23-Jul-2026).** A hand-rolled curl payload must include `context: {url, viewport:{w,h}, userAgent, browser, os, timestamp}` or the response is `{"ok":false,"error":"context object is required (...)"}`. Before the guard, the uncaught TypeError surfaced as a generic Cloudflare 1101 indistinguishable from a dead `GITHUB_TOKEN`, which cost a full diagnostic detour on 11-Jul-2026; a 1101 from this endpoint now points at real worker breakage, not a malformed test payload. Token rotation is `wrangler secret put GITHUB_TOKEN --env br360` from `backend/` (pipe the value via stdin).
